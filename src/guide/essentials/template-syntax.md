...
当 `isButtonDisabled` 为[真值](https://developer.mozilla.org/zh-CN/docs/Glossary/Truthy)或一个空字符串 (即 `<button disabled="">`) 时，元素会包含这个 `disabled` attribute。而当其为[假值](https://developer.mozilla.org/zh-CN/docs/Glossary/Falsy)时 attribute 将被忽略。
...
每个绑定仅支持**单一表达式**，也就是能计算成某个值的 JavaScript 代码。一个简单的判断方法是是否可以合法地写在 `return` 后面。
...
上面的例子将会在 DOM 内嵌模板中被转换为 `:[someattr]`。如果你的组件拥有 “someAttr” property 而非 “someattr”，这段代码将不会工作。单文件组件内的模版不受此限制。
